...
func (q *usdtTransferQ) Get() (*data.USDTTransfer, error) {
    ...
    stmt := q.sql.Select("*").From(usdtTransfersTableName)
...
func (q *usdtTransferQ) Update(transfer data.USDTTransfer) (*data.USDTTransfer, error) {
    ...
    var result data.USDTTransfer
    stmt := q.sql.Update(usdtTransfersTableName).SetMap(clauses).Where(sq.Eq{"id": transfer.ID}).Suffix("RETURNING *")
    err := q.db.Get(&result, stmt)
    if err != nil {
...
func (q *usdtTransferQ) Delete() error {
    stmt := q.sql.Delete(usdtTransfersTableName)
    err := q.db.Exec(stmt)
    if err != nil {
...
func (q *usdtTransferQ) Page(pageNumber, pageSize uint64) data.USDTTransferQ {
    offset := (pageNumber - 1) * pageSize
    q.sql = q.sql.Limit(pageSize).Offset(offset)
    return q
}